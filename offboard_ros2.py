...
class OffboardPolylineFollower(Node):
    def __init__(self):
        ...
        # ============================================================
        # SUES aerial pattern waypoints [x, y, z, yaw_deg]
        # 2x2 m frame divided into 4 cells (1x1 m each):
        #   top-left  (x:0-1, y:1-2): S   (drawn 1st, bottom->top)
        #   top-right (x:1-2, y:1-2): U   (drawn 2nd)
        #   bot-right (x:1-2, y:0-1): S   (drawn 3rd, top->bottom)
        #   bot-left  (x:0-1, y:0-1): E   (drawn 4th, bottom->top)
        # Reading order (L->R, top->bottom): S U E S
        ...
        # ============================================================
        self.waypoints = [
            # --- S (top-left cell, offset 0,1), reversed bottom->top ---
            [0.15, 1.25, 2.0, 0.0],
            [0.85, 1.25, 2.0, 0.0],
            [0.85, 1.55, 2.0, 0.0],
            [0.15, 1.55, 2.0, 0.0],
            [0.15, 1.85, 2.0, 0.0],
            [0.85, 1.85, 2.0, 0.0],
            # --- U (top-right cell, offset 1,1) ---
            [1.15, 1.85, 2.0, 0.0],
            [1.15, 1.25, 2.0, 0.0],
            [1.85, 1.25, 2.0, 0.0],
            [1.85, 1.85, 2.0, 0.0],
            # --- S (bottom-right cell, offset 1,0), top->bottom ---
            [1.85, 0.85, 2.0, 0.0],
            [1.15, 0.85, 2.0, 0.0],
            [1.15, 0.55, 2.0, 0.0],
            [1.85, 0.55, 2.0, 0.0],
            [1.85, 0.25, 2.0, 0.0],
            [1.15, 0.25, 2.0, 0.0],
            # --- E (bottom-left cell, offset 0,0), reversed bottom->top ---
            [0.85, 0.25, 2.0, 0.0],
            [0.15, 0.25, 2.0, 0.0],
            [0.15, 0.55, 2.0, 0.0],
            [0.85, 0.55, 2.0, 0.0],
        ...
        # Tuned for accuracy on the dense SUES polyline (segments ~0.3-0.7 m).
        ...
        # ------------------------------------------------------------
        self.pass_radius = 0.15            # tight for dense SUES waypoints
    ...
    def build_expanded_path(self):
        # SUES aerial pattern: use waypoints directly without gate expansion.
        # The gate pre/post expansion would distort the letter shapes, so the
        # SUES polyline is consumed as-is (one waypoint == one path vertex).
